...
// Debug-only legacy Capacitor bridge.
...
import com.audiobookshelf.app.util.SafeJsonObject
...
class AbsDatabase : Plugin() {
  ...
  override fun load() {
    ...
    ApiHandler.absDatabaseNotifyListeners = { event, data ->
      notifyListeners(event, JSObject(data.toString()))
    }
  ...
  fun getLocalLibraryItemByLId(call:PluginCall) {
    ...
    GlobalScope.launch(Dispatchers.IO) {
      val localLibraryItem = DeviceManager.dbManager.getLocalLibraryItemByLId(
        libraryItemId,
  ...
  fun syncLocalSessionsWithServer(call:PluginCall) {
    ...
    apiHandler.syncLocalMediaProgressForUser {
      ...
      val ownerConfig = DeviceManager.serverConnectionConfig
      val ownerLease = DeviceManager.captureConnectionLease(ownerConfig)

      if (savedSessions.isNotEmpty() && ownerConfig != null && ownerLease != null) {
        apiHandler.sendSyncLocalSessions(savedSessions, ownerConfig, ownerLease) { results, errorMsg ->
          if (results == null) {
            call.resolve(JSObject("{\"error\":\"$errorMsg\"}"))
          } else {
            val successfulIds = results.filter { it.success }.map { it.id }.toSet()
            val successful = savedSessions.filter { it.id in successfulIds }
            AbsLogger.info("AbsDatabase", "syncLocalSessionsWithServer: Removing ${successful.size} successfully synced sessions.")
            successful.forEach {
              DeviceManager.dbManager.removePlaybackSession(it)
            }
            call.resolve()
          }
        }
  ...
  fun updateLocalMediaProgressFinished(call:PluginCall) {
    ...
    localMediaProgress.serverConnectionConfigId?.let { configId ->
      if (DeviceManager.serverConnectionConfigId == configId) {
        ...
        val updatePayload = SafeJsonObject()
        updatePayload.put("isFinished", isFinished)
        apiHandler.updateMediaProgress(libraryItemId,episodeId,updatePayload) { _ ->
