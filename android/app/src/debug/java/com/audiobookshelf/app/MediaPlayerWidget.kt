...
// Debug-only phone widget; AAOS release builds do not compile this source.
...
class MediaPlayerWidget : AppWidgetProvider() {
  ...
  override fun onEnabled(context: Context) {
    ...
    initializeWidgetUpdater(context)
  }

  private fun initializeWidgetUpdater(context: Context) {
    ...
    DeviceManager.widgetUpdater = object : WidgetEventEmitter {
      override fun onPlayerChanged(pns: PlayerNotificationService) {
        ...
        val ids = appWidgetManager.getAppWidgetIds(
          ComponentName(appContext, MediaPlayerWidget::class.java)
        )
      ...
      override fun onPlayerClosed() {
        ...
        for (widgetId in ids) {
          updateAppWidget(
            ...
            appWidgetManager,
            widgetId,
            DeviceManager.deviceData.lastPlaybackSession,
            false,
            PlayerNotificationService.isClosed
          )
        }
...
internal fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int, playbackSession: PlaybackSession?, isPlaying:Boolean, isAppClosed:Boolean) {
  ...
  val wholeWidgetClickI = Intent(context, SettingsActivity::class.java).apply {
  ...
  val imageUri = playbackSession?.getCoverUri(context) ?: Uri.parse("android.resource://${BuildConfig.APPLICATION_ID}/drawable/icon")
  ...
  val playPauseResource = if (isPlaying) {
    R.drawable.shelfdrive_media_pause
  } else {
    R.drawable.shelfdrive_media_play
  }
